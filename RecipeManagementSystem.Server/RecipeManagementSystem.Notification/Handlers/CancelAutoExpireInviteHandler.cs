...
public class CancelAutoExpireInviteHandler(IScheduler scheduler, ILogger<CancelResendInviteHandler> logger)
    : IKafkaHandler<CancelAutoExpireInviteMessage>
{
    public async Task<Result> HandleAsync(CancelAutoExpireInviteMessage message, CancellationToken cancellationToken)
    {
        var jobKey = new JobKey($"job-{JobPrefixes.ExpiredInvite}-{message.InviteId}");
